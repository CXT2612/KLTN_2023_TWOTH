					...
					<Table className={ `table-striped table-bordered table-hover mb-9` } responsive>
						<thead>
							<tr>
								...
								<th className="text-nowrap text-center">Thời gian tạo</th>
							</tr>
						</thead>
						<tbody>
							{
								data?.length > 0 && data.map( ( item, key ) =>
								{
									return (
										< tr key={ key } className="table-product">
											...
											<td className="text-center d-flex justify-content-center align-items-center">{ customDate(item.created_at, 'DD/MM/yyyy HH:mm') }</td>
